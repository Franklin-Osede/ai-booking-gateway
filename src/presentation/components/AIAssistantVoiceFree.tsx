                    ...
                    {msg.sender === "bot" && !msg.isCalendar && !msg.isFinalCard ? (
                        ...
                        <div className="bg-white border border-gray-100 rounded-3xl p-5 shadow-sm w-full">
                           ...
                           <div className="grid grid-cols-7 gap-y-2 gap-x-1 text-center text-[12px] font-semibold mb-4">
                             {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((d) => {
                               ...
                               return (
                                 <button key={d} disabled={isPast || isRed} onClick={() => { setSelectedDate(d); setSelectedTime(""); }} className={btnClass} style={selectedDate === d ? { backgroundColor: color, color: getContrastColor(color) } : {}}>{d}</button>
                               )
                             })}
                           </div>
                           {selectedDate && (
                      ...
                      <div className="w-full pt-1 pb-2">
                         <div className="bg-white border text-center border-gray-100 rounded-3xl p-6 shadow-[0_10px_40px_rgba(0,0,0,0.06)] w-full relative overflow-hidden">
                           ...
                           <h3 className="font-extrabold text-[17px] text-gray-900 mb-1 tracking-tight">{isEng ? "Appointment Requested!" : "¡Cita Solicitada!"}</h3>
                           <p className="text-[12px] text-gray-500 mb-6 font-medium leading-relaxed px-2">{isEng ? "We have sent an email with all the details and preparation guidelines for your visit." : "Hemos enviado un email con todos los detalles e información para preparar tu visita a la clínica."}</p>
                           ...
                           <div className="bg-gray-50/80 rounded-2xl p-4 mb-6 text-left space-y-3.5 border border-gray-100">
                             {selectedDoctor && (
                               <div className="flex flex-col gap-1 border-b border-gray-200/60 pb-3">
                                 <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{isEng ? "Advisor" : "Asesora"}</span>
